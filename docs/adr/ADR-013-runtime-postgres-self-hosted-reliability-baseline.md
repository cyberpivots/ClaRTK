...
- The current runtime integration is real for auth/profile data, and the gateway now persists replay plus transport-capture evidence, but validated fused solver publication is still incomplete.
...
- Dedicated runtime role/bootstrap automation, compose-backed WAL archiving, base-backup capture, observability collection, replay-backed gateway persistence, and live transport acquisition loops are now repo-owned surfaces.
- Disposable restore-drill automation, non-compose production config templates, and environment-specific host-managed config rendering are now repo-owned surfaces.
